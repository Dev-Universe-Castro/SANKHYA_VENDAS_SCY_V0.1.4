...
interface Contrato {
  ...
  ATIVO: boolean
  IS_SANDBOX?: boolean // Added this line
}
...
export default function ContratosPage() {
  ...
  const [formData, setFormData] = useState<Partial<Contrato>>({
    ...
    GEMINI_API_KEY: "",
    ATIVO: true,
    IS_SANDBOX: true // Added this line
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Construct the payload including IS_SANDBOX
  ...
  const handleEdit = (contrato: Contrato) => {
    ...
    setFormData({
      ...
      IS_SANDBOX: contrato.IS_SANDBOX !== undefined ? contrato.IS_SANDBOX : (contrato as any).IS_SANDBOX === 'S' // Handle potential string 'S' from backend
    })
    setIsEditing(true)
    setIsModalOpen(true)
  }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Ambiente</TableHead> {/* Added header for Sandbox */}
              ...
              <TableBody>
                {contratos.map((contrato) => (
                  <TableRow key={contrato.ID_EMPRESA}>
                    ...
                    <TableCell> {/* Cell for Sandbox status */}
                ...
                {contratos.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground"> {/* Increased colSpan */}
