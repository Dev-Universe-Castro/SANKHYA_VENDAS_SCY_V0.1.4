  ...
  for (const cabecalho of cabecalhos) {
    try {
      ...
      if (exists) {
        ...
        // Converter a data do formato ISO ou DD/MM/YYYY para o formato correto
        let dtnegFormatted = null;
        if (cabecalho.DTNEG) {
          try {
            // Tentar parsear a data (pode vir como '2024-01-15 00:00:00' ou '15/01/2024')
            const dtnegStr = String(cabecalho.DTNEG).trim();

            // Se estiver no formato ISO (YYYY-MM-DD ou YYYY-MM-DD HH:MM:SS)
            if (dtnegStr.includes('-')) {
              const datePart = dtnegStr.split(' ')[0]; // Pega apenas a parte da data
              const [year, month, day] = datePart.split('-');
              dtnegFormatted = `${day}/${month}/${year}`;
            } 
            // Se já estiver no formato DD/MM/YYYY
            else if (dtnegStr.includes('/')) {
              dtnegFormatted = dtnegStr.split(' ')[0]; // Remove hora se houver
            }
          } catch (error) {
            console.log(`⚠️ [Sync] Erro ao converter data DTNEG para NUNOTA ${cabecalho.NUNOTA}: ${cabecalho.DTNEG}`);
            dtnegFormatted = null;
          }
        }
        ...
        await connection.execute(
          `INSERT INTO AS_CABECALHO_NOTA (
            ...
            :codvend, :vlrnota, ${dtnegFormatted ? "TO_DATE(:dtneg, 'DD/MM/YYYY')" : 'NULL'}, :tipmov,
